...
def predict_chars(images):
  ...
  for image_name in images:
        # The images are already grayscale, so keep them that way
        # Just resize to the required dimensions
  ...
  # Don't use the inception preprocessing since you need grayscale (1 channel) images
  # Inception preprocessing is for RGB images, which is causing the error
  # test_images = tf.keras.applications.inception_resnet_v2.preprocess_input(test_images, data_format=None)
  
  print(test_images.shape)  # Should print something like (n, 75, 75, 1)
...
# Load image
img = cv2.imread('F:/final_bramhi/OCR/recognize/b1.jpg')
